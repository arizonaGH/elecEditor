...
Format.prototype.refresh = function()
{
	...
	else
	{
		...
		else
		{
            ...
            // Arrange
            mxUtils.write(label3, mxResources.get('properties'));
            div.appendChild(label3);

            var propertyPanel = div.cloneNode(false);
            propertyPanel.style.display = 'none';
            this.panels.push(new PropertiesPanel(this, ui, propertyPanel));
...
PropertiesPanel.prototype.init = function()
{
    ...
    //show property panel if only one element was selected.
    if(graph.getSelectionCells().length != 1)
	{
		return;
	}

    var properties = graph.getSelectionCell().getValue();
    if(typeof properties != "object")
	{
		return;
	}

	var elementType = properties.getAttribute("type");
    if(elementType == null)
	{
		return;
	}
 ...
 * add input widget
 */
PropertiesPanel.prototype.addBasicParameter = function(div, cfg)
{
    div.appendChild(this.createTitle(mxResources.get('basic')));

	for(let item of cfg)
	{
		if(item.type == "input")
		{
            this.addInput(div, item.label, item.propertyName);
		}
		if(item.type == "select")
		{
 ...
 * add select widget
 */
PropertiesPanel.prototype.addRuntimeParameter = function(div, cfg)
{
    div.appendChild(this.createTitle(mxResources.get('runtime')));
 ...
 * add input widget
 */
PropertiesPanel.prototype.addInput = function(div, label, propertyName)
{
    var graph = this.editorUi.editor.graph;

    var row = document.createElement('div');
    row.style.padding = '6px 0px 1px 0px';
    ...
    function update(evt)
    {
        ...
        {
            graph.getSelectionCell().getValue().setAttribute(propertyName, input.value);
        }
	...
	var listener = mxUtils.bind(this, function(sender, evt, force)
	{
		...
		if (force || input != document.activeElement)
		{
			if(graph.getSelectionCell() != null)
			{
                let obj = graph.getSelectionCell().getValue();
                if(obj != null)
				{
                    let value = obj.getAttribute(propertyName);
...
PropertiesPanel.prototype.addSelect = function(div, label, propertyName, sourceType, options)
{
    ...
    var listener = mxUtils.bind(this, function(sender, evt, force)
    {
        let value = graph.getSelectionCell().getValue().getAttribute(propertyName);
        if(value != null)
		{
            select.value = value;
		}
